...
async function loop(){
  console.log('VPN Server (no-delete client version)');
  while(true){
    try{
      ...
      for(const reqFile of reqFiles){
        ...
        try{
          ...
          // همیشه با sha null می‌نویسیم (در صورت وجود فایل قبلی، آن را حذف نمی‌کنیم)
          await apiPut(`res_${uuid}.json`,resContent,`Response`,null);
          // درخواست را پاک کن
          await apiDelete(`req_${uuid}.json`,reqFileData.sha,`Processed`);
          console.log(`Done ${uuid.slice(0,8)}`);
        }catch(e){
          console.error('Error:',e.message);
          try{
            const errContent=JSON.stringify({status:'error',error:e.message,timestamp:Date.now()});
            await apiPut(`res_${uuid}.json`,errContent,`Error`,null);
            await apiDelete(`req_${uuid}.json`,reqFile.sha,`Error`);
          }catch(e2){}
        }
      }
      // پاکسازی فایل‌های res قدیمی‌تر از ۵ دقیقه
      const now = Date.now();
